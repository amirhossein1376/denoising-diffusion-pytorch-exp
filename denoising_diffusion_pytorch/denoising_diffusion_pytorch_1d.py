...
class LinearAttention(Module):
    ...
    def forward(self, x, use_attention=True):
        ...
        # NOTE: compute attention the same way for tensor masks as for scalar True,
        # then blend per-sample. This ensures identical behavior when mask is all True.
        b, c, n = x.shape
        qkv = self.to_qkv(x).chunk(3, dim = 1)
        q, k, v = map(lambda t: rearrange(t, 'b (h c) n -> b h c n', h = self.heads), qkv)

        # compute standard attention output (same for scalar True)
...
class Attention(Module):
    ...
    def forward(self, x, use_attention=True):
        # support per-sample mask: behave like scalar True then blend per-sample
        b, c, n = x.shape
        qkv = self.to_qkv(x).chunk(3, dim = 1)
        q, k, v = map(lambda t: rearrange(t, 'b (h c) n -> b h c n', h = self.heads), qkv)
...
class GaussianDiffusion1D(Module):
    ...
    def forward(self, img, *args, **kwargs):
        ...
        # If no steps_without_attention are requested, pass a scalar True so
        # attention modules behave exactly as before.
